...
Cloud Nativeエコシステム全体を体系的に学ぶ最も効果的なパスである。
...
- **kind**: ローカル環境でのKubernetesクラスタ。リソース制約があるため一部ハンズオンに制限あり
- **minikube**: ローカル環境でのシングルノードクラスタ
- **kubeadm**: 自前で構築するクラスタ（付録Aに構築手順を記載）
- **他のマネージドK8s**: EKS、GKE、AKS等
...
| ツール | 用途 | 推奨バージョン |
|--------|------|--------------|
| kubectl | Kubernetesクラスタの操作 | v1.34以降 |
| kustomize | マニフェストのカスタマイズ | v5.x |
| helm | パッケージマネージャ | v3.x |
| git | バージョン管理 | v2.x |
