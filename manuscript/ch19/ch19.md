...
- **Knative**: サーバーレスワークロード管理。イベント駆動アーキテクチャの構築
- **Dapr**: 分散アプリケーションランタイム。サービス間通信、状態管理、Pub/Subの抽象化
- **FinOps**: クラウドコスト管理。Kubecostによるリソースコストの可視化と最適化
- **Kyverno**: Kubernetes Native Policy Engine。OPA/Gatekeeperの代替
- **Argo Workflows**: Kubernetes上のワークフローエンジン。データパイプラインやMLパイプライン
